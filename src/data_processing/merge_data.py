...
def merge(type='training'):
    ...
    for ticker in tickers:
        print(ticker)
        if type == 'training':
            data = read_files.read_preprocessed_training_data(ticker)
        elif type == 'val':
            data = read_files.read_preprocessed_val_data(ticker)
        elif type == 'test':
            data = read_files.read_preprocessed_test_data(ticker)
        ticker = ticker.split('.')[0]
        for col in data.columns:
            data.rename(columns={col : (col.lower(), ticker)}, inplace=True)
